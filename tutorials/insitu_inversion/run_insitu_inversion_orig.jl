...
Pkg.instantiate()
#using Revise
using SindbadPriming
# using SindbadPriming.Sindbad
# using SindbadPriming.Sindbad.Setup.Dates
# using SindbadPriming.Plots
# using SindbadPriming.Sindbad.Visualization
...
# currently forcing is set in settings/forcings.json to an S3 bucket and is downloaded on demand
#    on BGC cluster need to use local version and set the path in forcings.json
...
experiment_json = joinpath(@__DIR__,"../setups/WROASTED_HB/","experiment_insitu.json");
experiment_name = "WROASTED_HB_inversion_CMAES";
replace_info    = Dict("experiment.basics.time.date_begin" => "$(begin_year)-01-01",
    "experiment.basics.domain" => domain,
    "experiment.basics.name" => experiment_name,
    "experiment.basics.time.date_end" => "$(end_year)-12-31",
